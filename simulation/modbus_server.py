...
from . import config
from .solar_model import BatteryModel, GridModel, InverterModel, SolarModel
    ...
    async def run_loop(self):
        ...
        while True:
            try:
                ...
                export_limit_raw = slave_context.getValues(
                    3, config.REG_GRID_EXPORT_LIMIT, count=1
                )[0]
                self.grid.export_limit = float(export_limit_raw)

                # Get Inverter Enable Coil (Coil 0)
                inverter_enable = slave_context.getValues(
                    1, config.COIL_INVERTER_ENABLE, count=1
                )[0]
                self.inverter.status = 1 if inverter_enable else 0

                # Get Defence Mode Coil (Coil 1)
                defence_mode = slave_context.getValues(
                    1, config.COIL_DEFENCE_MODE, count=1
                )[0]
                ...
                if defence_mode:
                    ...
                    if self.grid.export_limit < 100.0:
                        logger.warning(
                            "DEFENCE MODE: Detected malicious Export Limit change! Reverting..."
                        )
                ...
                slave_context.setValues(
                    3, config.REG_SOLAR_OUTPUT, [int(solar_power * 10)]
                )

                # Inverter Status
                slave_context.setValues(
                    3, config.REG_INVERTER_STATUS, [self.inverter.status]
                )
                ...
                # Active Power (x10)
                slave_context.setValues(
                    3, config.REG_GRID_POWER_ACTIVE, [int(actual_export * 10)]
                )

                # Irradiance
                slave_context.setValues(
                    3, config.REG_IRRADIANCE, [int(self.solar.irradiance)]
                )

                logger.debug(
                    f"Sim Update: Solar={solar_power:.2f}kW, Export={actual_export:.2f}kW, SoC={soc:.1f}%"
                )
...
async def main():
    ...
    logger.info(
        f"Starting Modbus TCP Server on {config.MODBUS_HOST}:{config.MODBUS_PORT}"
    )
    await StartAsyncTcpServer(
        context, identity=identity, address=(config.MODBUS_HOST, config.MODBUS_PORT)
    )
